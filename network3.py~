...
class ConvPoolLayer(object):
    ...
    #### Defining the binarizing functions 
    def binarize_weight(self):
        sum = 0.0
        for i in range(filter_shape[0]):
            for j in range(filter_shape[1]):
                for k in range(filter_shape[2]):
                    for l in range(filter_shape[3]):
                        sum = sum + abs(self.w[i][j][k][l])
                        if self.w[i][j][k][l]==0:
                            self.w[i][j][k][l]=1
        m= ((filter_shape[0])*(filter_shape[1])*(filter_shape[2])*(filter_shape[3])) 
        W=sum/m
        alpha = theano.shared(W,'alpha')
